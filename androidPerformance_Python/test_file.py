    ...
    for data in open('GCFile.txt'):
        for i in data.split(' '):
            if AppPID == i and 'freed' in data:
                ...
                if '+' in number1:
                    GC_time = int(filter(str.isdigit, number1.split('+')[0])) + int(
                        filter(str.isdigit, number1.split('+')[1]))
...
if __name__ == "__main__":
    ...
    GC_Freed_list, average_GC_Freed, GC_count_list, GC_per_list, average_GC_per, GC_time_list, average_GC_time = analysisGCFile(
        app_pid)
