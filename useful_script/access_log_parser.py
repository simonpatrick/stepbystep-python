...
def getTimestamp(st):
    ...
    return str(time.mktime((int(yyyy), int(mm), int(dd), int(hh), int(ii), int(ss),0,0,0))),int(yyyy),int(mm),int(dd)
...
def main():
    ...
    p.add_option('--size', '-s', type="int", dest="size", default=10,help="choose the number of record to store in every log")
